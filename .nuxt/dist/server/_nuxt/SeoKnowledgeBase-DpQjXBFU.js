    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (seoContent.value.faq.length > 0) {
        ...
        ssrRenderList(seoContent.value.faq, (item, index) => {
          _push(`<div class="border-l-2 border-primary pl-4 py-1"><h3 class="text-lg font-semibold mb-2">${ssrInterpolate(item.question)}</h3><p class="text-muted-foreground">${item.answer ?? ""}</p></div>`);
        });
        _push(`<!--]--></div>`);
      } else {
        _push(`<!---->`);
...
//# sourceMappingURL=SeoKnowledgeBase-DpQjXBFU.js.map
